...
**The AVS result is advisory — do not branch on it as if it were the outcome.** `transactions_sale`, `transactions_auth`, `transactions_get` and `transactions_inspect` return the address check in the **top-level** `avsResponse` object (`{responseCode, action, group, result, codeDescription, …}`), or `null` when AVS is off for the merchant/processor. An **Approved** transaction can still carry `result: "Failed"` when the merchant's AVS `action` is `"Allow"` — so a failed AVS result is not a failed charge, and retrying on it double-charges. Read `status`/`responseDescription` for the outcome. Note also that `transactionReceipt.avsResponse` may be `null` even when the top-level field is populated; read the top-level one.
