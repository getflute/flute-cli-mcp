...
impl FluteServer {
    #[tool(
        description = "List mobile payment devices. Records use `deviceId`, not `id`. Safe to retry."
    )]
    ...
    #[tool(
        description = "Mint a Tap-to-Pay JWT for a device. Returns a `tap_to_pay_jwt` envelope. Idempotent read."
    )]
    pub async fn devices_ttp_jwt(
        &self,
        Parameters(p): Parameters<DeviceId>,
    ) -> Result<CallToolResult, McpError> {
        let mut args = self.base_args();
        args.extend([
            "devices".into(),
    ...
    pub async fn devices_register(
        &self,
        Parameters(p): Parameters<DeviceRegister>,
    ) -> Result<CallToolResult, McpError> {
        if let Some(blocked) = self.guard_write("devices_register") {
            return Ok(blocked);
        }
        let mut args = self.base_args();
    ...
    pub async fn devices_ttp_activate(
        &self,
        Parameters(p): Parameters<Id>,
    ) -> Result<CallToolResult, McpError> {
        if let Some(blocked) = self.guard_write("devices_ttp_activate") {
            return Ok(blocked);
        }
        let mut args = self.base_args();
